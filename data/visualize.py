            ...
            for root, dirs, files in os.walk(raw_dir, topdown=False):
                for file in files:
                    ...
                    if ext == 'wav':
                        ...
                        np.savetxt(os.path.join(root, name + ".txt"), vector_feature)
                        ax.set_title(name)
            ...
            for root, dirs, files in os.walk(raw_dir, topdown=False):
                for file in files:
                    ...
                    if ext == 'wav':
                        ...
                        print(vector_feature.min())
                        print(vector_feature.max())
                        fig, ax = plt.subplots()
                        t = np.arange(0, vector_feature.shape[0], 1)
                        f = np.arange(0, vector_feature.shape[1], 1)
                    ...
                    if ext == 'dpf':
                        print(file)
                        filename = os.path.join(root, file)
                        vector_feature = spectrogram.generate(filename,num_context,istxt=True)
                        fig, ax = plt.subplots()
                        t = np.arange(0, vector_feature.shape[0], 1)
                        f = np.arange(0, vector_feature.shape[1], 1)
                        ax.pcolormesh(t,f, vector_feature.T, cmap="jet")
