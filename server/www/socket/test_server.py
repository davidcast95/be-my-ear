...
n_hidden_5 = int(config['forward-net']['n_hidden_5'])
...
print("Hidden Layer 5 : " + str(n_hidden_5))
print("Charset : " + str(n_hidden_6))
print('\n')
...
with tf.device('/gpu:0'):
    ...
    with tf.name_scope('biRNN'):
        ...
        w5 = tf.get_variable('fc5_w', [n_hidden_3, n_hidden_5], tf.float32,
                                  tf.random_normal_initializer(mean, std))
        b5 = tf.get_variable('fc5_b', [n_hidden_5], tf.float32,
                                  tf.random_normal_initializer(mean, std))
        # reshape to [batchsize * timestep x num_cepstrum]
    ...
    with tf.name_scope('logits'):
        w6 = tf.get_variable('fc6_w', [n_hidden_5, n_hidden_6], tf.float32,
                                  tf.random_normal_initializer(mean, std))
        b6 = tf.get_variable('fc6_b', [n_hidden_6], tf.float32,
                                  tf.random_normal_initializer(mean, std))

        h6 = tf.add(tf.matmul(h5_bn, w6), b6)
        ...
        for file in files:
            if (len(file.split('.')) == 2):
                ...
                if file[0] != '_' and ext == 'npy':
                    ...
                    os.remove(filename, os.path.join(root, file))
            ...
            if (len(file.split('.')) == 2):
                ...
                if (file[0] != '_' and ext == 'wav'):
                    if type == 'mfcc':
                        ...
                        os.renames(os.path.join(root,file),os.path.join(root,'_' + file))
...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
    ...
    if os.path.exists(os.path.join(model_data, 'checkpoint')):
        start = timer()
    ...
    while True:
        ...
        wavfile = wave.open(os.path.join('..','wav_data',str(addr)+'.wav'), 'w')
        ...
        while True:
            ...
            if (len(data) == 0):
                ...
                run_model(sess, clientsocket)
                wavfile.close()
                print("saved : "+ str(addr+'.wav'))
                wavfile = wave.open(os.path.join('..', 'wav_data', str(addr) + '.wav'), 'w')
                wavfile.setparams((1, 2, 16000, 0, 'NONE', 'not compressed'))
                length = 0
                break
