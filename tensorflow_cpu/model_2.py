...
sys.path.append("../")
from modules.model.classes import Model, Datasets
...
if len(sys.argv) == 3:
    ...
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
        os.makedirs(os.path.join(log_dir, 'training'))
        os.makedirs(os.path.join(log_dir, 'testing'))

    model = Model()
    session.load_from_file(os.path.join(model_dir,'session.ini'))
    model.load_from_file(os.path.join(model_dir,'model.ini'))
    model.create_network()

    datasets = Datasets()
    datasets.load_from_folder(training_dir, testing_dir)

    model.restoring(checkpoint_dir)

    model.feed(10,report_dir,checkpoint_dir,datasets)
