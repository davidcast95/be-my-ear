...
if len(sys.argv) == 3:
    ...
    with tf.device('/cpu:0'):
        ...
        with tf.name_scope('conv-net'):
            # reshape to [batchsize * timestep x num_cepstrum]
            reshape_input_batch = tf.reshape(input_batch, [-1, num_cep])

            #transpose to [num_cepstrum x batchsize * timestep]
            transpose_input_batch = tf.transpose(reshape_input_batch)

            f, t = transpose_input_batch.shape

            reshape_conv_input_batch = tf.reshape(transpose_input_batch, [-1, f, t, 1])

            conv1 = tf.layers.conv2d(
                inputs=reshape_conv_input_batch,
            ...
            print(conv1)
            poll1 = tf.layers.max_pooling2d(inputs=conv1, pool_size=[2, 2], strides=2)
            print(poll1)
            h1_bn = batch_norm(poll1,'pool1_bn',tf.cast(is_training,tf.bool))
            print(h1_bn)

        with tf.name_scope('biRNN'):
            with tf.variable_scope('fc4') as fc3:
                w4 = tf.Variable(tf.random_normal([n_hidden_2, n_hidden_4], mean, std, tf.float32), name='fc4_w')
                b4 = tf.Variable(tf.random_normal([n_hidden_4], mean, std, tf.float32), name='fc4_b')

            # reshape to [time x batchsize x 2*n_hidden_3]
            h2 = tf.cond(is_training,
                         lambda: tf.reshape(h1_bn, [-1, training_batch, n_hidden_2]),
                         lambda: tf.reshape(h1_bn, [-1, testing_batch, n_hidden_2]) )

            forward_cell =  GRUCell(n_hidden_3)
            backward_cell = GRUCell(n_hidden_3)
            ...
            #reshape to [batchsize * timestep x num_cepstrum]
            h3 = tf.reshape(outputs,[-1,2 * n_hidden_3])
            h4 = tf.add(tf.matmul(h3, w4), b4)
            h4_bn = batch_norm(h4, 'fc4_bn', tf.cast(is_training, tf.bool))
            h4 = tf.minimum(tf.nn.relu(h4_bn), relu_clip)

        with tf.name_scope('logits'):
            with tf.variable_scope('fc5') as fc6:
            ...
            #reshape to [time x batchsize x n_hidden_5]
            logits = tf.cond(is_training,
                             lambda :tf.reshape(h5, [-1, training_batch, n_hidden_5]),
                             lambda :tf.reshape(h5, [-1, testing_batch, n_hidden_5]))
