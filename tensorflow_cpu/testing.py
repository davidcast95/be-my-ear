...
csv_fields = ['checkpoint','batch','ctc_loss','decoded_text','target_text','WER']
    ...
    for root, dirs, files in os.walk(training_dir, topdown=False):
        for file in files:
            if file[0] == '_':
                ...
                testing_dataset.append(np.load(os.path.join(training_dir, file)))
        ...
        for root, dirs, files in os.walk(checkpoint_dir, topdown=True):
            for dir in dirs:
                if dir[0] == 'D' and dir[1] == 'M' and dir[2] == 'C' and dir[3] == '-':
                    ...
                    saver.restore(sess,os.path.join(os.path.join(checkpoint_dir, checkpoint), 'tensorflow_1.ckpt'))
                    ...
                    for i in range(int(len(testing_dataset))):
                        csv_values = []
                        print ("Checkpoint : "+checkpoint)
                        report.write("batch #"+str(i))
                        csv_values.append(checkpoint)
                        csv_values.append(i)
                        #get batch
                        ...
                        print ("Target text :")
                        report.write(data_rep.indices_to_text(target_dataset[(i * batch):(i * batch) + batch][0]))
